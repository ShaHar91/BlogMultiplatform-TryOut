...
import org.example.blogmultiplatform.styles.NewsletterInputStyle
...
import org.example.blogmultiplatform.utils.subscribeToNewsletter
import org.example.blogmultiplatform.utils.validateEmail
...
fun NewsletterSection(
    ...
    if (invalidEmailPopup) {
        MessagePopup(
            ...
            onDialogDismiss = { invalidEmailPopup = false }
        )
    }
    ...
    if (subscribedPopup) {
        MessagePopup(
            ...
            onDialogDismiss = { subscribedPopup = false }
        )
    }
...
fun SubscriptionForm(
    ...
    Input(
        ...
        attrs = NewsletterInputStyle.toModifier()
            ...
            .backgroundColor(Theme.Grey.rgb)
            .padding(leftRight = 24.px)
            .margin(right = if (vertical) 0.px else 20.px, bottom = if (vertical) 20.px else 0.px)
            .fontFamily(FONT_FAMILY)
            .fontSize(16.px)
            .borderRadius(100.px)
        ...
        attrs = Modifier
            .onClick {
                ...
                if (validateEmail(email = email)) {
                    scope.launch {
                        onSubscribed(subscribeToNewsletter(Newsletter(email)))
                    }
                } else {
                    onInvalidEmail()
                }
